...
class SqlAuthRepository:
    ...
    def _to_domain(row: UserRow) -> Account:
        return Account(
            ...
            email=row.email or "", role=Role(row.role), is_active=row.is_active,
            provider=AuthProvider(row.provider), max_menus=row.max_menus,
    ...
    async def create_account(
        ...
        user = UserRow(
            ...
            email_verified_at=email_verified_at, max_menus=max_menus, created_at=now,
        )
        self._s.add(user)
        await self._s.flush()
    ...
    async def set_limits(self, user_id: UUID, *, max_menus: int | None) -> bool:
        row = await self._s.get(UserRow, user_id)
        if row is None:
            return False
        row.max_menus = max_menus
        await self._s.flush()
        return True
