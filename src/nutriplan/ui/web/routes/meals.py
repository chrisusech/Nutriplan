...
def _back_to_meal(
    ...
    *,
    ...
    return RedirectResponse(
        f"/?dia={dia}&nota={quote(slot)}{extra}",
        status_code=303,
    )
...
async def eating_out_save(
    ...
    try:
        ...
        # Solo hace falta resolver lo que YA está en el día para recalcularlo.
        # Pedir el catálogo entero además de innecesario sería incorrecto: un
        # alimento que entró por un cambio de plato no está en lo listable, y el
        # día se quedaría sin sus macros.
        en_el_dia = [i.food_id for m in day.meals for i in m.items if i.food_id]
        foods = {f.id: f for f in await repos.foods.get_by_ids(en_el_dia)}
        targets = await repos.targets.latest_for_client(client.id)
        if targets is None:
