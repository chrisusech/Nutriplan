...
Genera su semana, la vive, la cierra (peso + notas + una frase) y solo entonces
la app le arma la siguiente. Es el corazón de la fase 2: lo que se pide aquí es
exactamente lo que alimenta el plan de la semana que viene.
"""
...
def test_con_la_semana_ya_generada_el_cierre_pide_comentario(app) -> None:
    _generar(app)
    html = app.get("/check-in").text
    assert "¿Cómo te fue esta semana?" in html
    assert "Revisa tus alimentos" in html
    assert "kcal esta semana" in html
    assert "fuera" in html.lower()
    assert "Calificar platos" not in html
...
def test_no_puede_pedir_otra_semana_sin_cerrar_la_que_vive(app) -> None:
    ...
    assert "cómo te fue" in bloqueada.text.lower()
    assert 'href="/check-in"' in bloqueada.text


def test_no_se_cierra_la_semana_con_un_comentario_de_dos_letras(app) -> None:
    """Sin una frase de verdad no hay nada que interpretar."""
    _generar(app)
    resp = app.post("/check-in", data={"weight_kg": "61.4", "comentario": "ok"})
    assert resp.status_code == 200
    assert "cómo te fue" in resp.text
