...
def test_la_ia_puede_ponerle_nombre_a_un_plato(nutrition_config) -> None:
    ...
    critique = _critique(schema, [{
        "day_index": 0, "slot": "desayuno",
        "dish_name": "Tostada de huevos con aguacate",
        "swap_out_food_id": None, "swap_in_food_id": None,
...
def test_ponerle_nombre_a_un_plato_no_le_mueve_un_solo_gramo(nutrition_config) -> None:
    ...
    critique = _critique(schema, [
        {"day_index": d.day_index, "slot": m.slot.value, "dish_name": "Un nombre",
         "swap_out_food_id": None, "swap_in_food_id": None, "reason": ""}
        for d in plan.days for m in d.meals
    ])
    ...
    despues = {
        (d.day_index, m.slot): [(i.food_id, i.grams) for i in m.items]
        for d in out.days for m in d.meals
    }
...
def test_la_ia_no_puede_cambiar_un_plato_si_eso_saca_el_dia_de_tolerancia(
    ...
    proteina = next(
        foods[i.food_id] for i in almuerzo.items
    ...
    schema = build_critique_schema(list(foods.values()))
    critique = _critique(schema, [{
        "day_index": 0, "slot": "almuerzo", "dish_name": "Almuerzo de fruta",
    ...
    nuevo_almuerzo = next(
        m for m in out.days[0].meals if m.slot is MealSlot.LUNCH
    )
    assert [i.food_id for i in nuevo_almuerzo.items] == [
        i.food_id for i in almuerzo.items
...
def test_un_swap_hacia_un_alimento_que_no_esta_en_la_comida_se_ignora(
    ...
    schema = build_critique_schema(list(foods.values()))
    critique = _critique(schema, [{
        "day_index": 0, "slot": "desayuno", "dish_name": "Desayuno",
...
def test_la_comida_libre_no_se_toca_ni_se_renombra(nutrition_config) -> None:
    ...
    libre = cena.model_copy(update={
        "items": [], "is_free_meal": True,
        "computed": MacroTargets(kcal=0, protein_g=0, carb_g=0, fat_g=0),
    })
    domingo = domingo.model_copy(update={
        "meals": [libre if m.slot is MealSlot.DINNER else m for m in domingo.meals]
    })
    days = [*plan.days[:6], domingo]

    schema = build_critique_schema(list(foods.values()))
    critique = _critique(schema, [{
        "day_index": 6, "slot": "cena", "dish_name": "Cena elegante",
        "swap_out_food_id": None, "swap_in_food_id": None, "reason": "",
    }])
...
def test_el_schema_no_deja_ni_nombrar_un_alimento_prohibido() -> None:
    ...
    try:
        schema.model_validate({"meals": [{
            "day_index": 0, "slot": "almuerzo", "dish_name": "x",
            "swap_out_food_id": ajeno, "swap_in_food_id": ajeno, "reason": "",
        }]})
    except Exception:
        return
...
def test_un_cambio_razonable_si_se_aplica_y_el_dia_sigue_cuadrando(
    ...
    schema = build_critique_schema(list(foods.values()))
    critique = _critique(schema, [{
        "day_index": 0, "slot": "almuerzo", "dish_name": "Tilapia con arroz",
...
async def test_sin_llm_el_menu_se_genera_igual_solo_que_sin_nombres(
    ...
    out = await refine_week(
        days=plan.days, client=_cliente(), allowed=list(foods.values()),
        config=nutrition_config, daily=_daily(), llm=None,
        prompts_dir=PROMPTS, model="offline",
    )
    assert out is None
...
async def test_si_el_proveedor_falla_el_menu_sobrevive_sin_refinar(
    ...
    out = await refine_week(
        days=plan.days, client=_cliente(), allowed=list(foods.values()),
        config=nutrition_config, daily=_daily(), llm=_Caido(),
